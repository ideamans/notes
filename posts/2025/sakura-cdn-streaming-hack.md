...
それは、**ストリーミングデータの収集基盤としてほぼ無料で使う**という方法だ。
...
通常、ユーザーの行動データを大量に収集するには、[AWS Kinesis Data Firehose](https://aws.amazon.com/jp/firehose/)のようなストリーミングデータ基盤を使う。しかしこれらはリクエスト数に応じて課金されるため、トラフィックが増えるとコストも膨らむ。
...
ブラウザのセキュリティ制約（同一オリジンポリシー）により、JavaScriptは基本的に配信元と同じオリジンにしかデータを送信できない。これが意外とCDN活用の追い風になる。
...
[Ranklet4](https://ranklet4.com/)は、Webサイトのアクセスランキングを提供するサービスになる。

ランキングの表示回数や、どの順位がクリックされたかといったインタラクションデータを収集する必要がある。月間数億PV規模のトラフィックを扱うが、このログ収集にかかるコストは**月額数百円〜数千円程度**で済んでいる。

同等のトラフィックをFirehoseで処理すれば、コストは桁違いに膨らむだろう。

## AWS Firehoseとの比較

| 項目 | AWS Firehose | ウェブアクセラレータ |
|------|--------------|---------------------|
...
Firehoseは柔軟で高機能だが、トラフィックに比例してコストが増える。ウェブアクセラレータは制約があるものの、大量のリクエストを低コストでさばける。
...
やや「ずるい」使い方ではあるが、公式に禁止されているわけではない。大量のユーザーデータを収集したいが、Firehoseのようなサービスのコストが気になる——そんなときの選択肢として覚えておいて損はない。
