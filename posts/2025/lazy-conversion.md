...
description: 次世代画像フォーマット変換を「事前変換」と「オンデマンド変換」で比較し、ECサイトのように画像数が多い場合はオンデマンド変換が現実的であることを解説。さらに初回アクセスの遅延とサーバー過負荷を回避する「オンデマンド遅延変換」の設計思想とその優位性を論じる。
...
小規模サイトであればキャッシュはすぐ有効に機能する。一方、EC サイトはロングテール上に多数のアクセス頻度の低いページを抱える傾向だ。全体として画像への初回アクセスが多いので、キャッシュが効くからと割り切ることはできない。
