...
description: 「10GBに収まるよう古いファイルを削除する」という目標容量ベースのクリーニングをGo言語で実装したOSSツールを紹介。ブロックサイズ基準の計算、省メモリ設計、並列処理による高速化など、大量ファイルを扱うサーバー運用の実用的な工夫を解説する。
...
バックアップファイルは最終更新時刻の新しさで価値を測る。キャッシュファイルでは最終アクセス時刻の新しさが価値の基準となる。

そこで`-a`オプションを指定すると最終アクセス時刻を基準に古いファイルを判定する。

### ブロックサイズベースの計算

このツールは論理的なファイルサイズではなく、ディスク上で占有するブロックサイズを基準にして使用量を計算する。
