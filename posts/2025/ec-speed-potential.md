...
description: 通販サイト約29万セッションのデータを分析すると、全注文の57%が平均LCP 1.19秒以下のユーザーによるものと判明。収益最大化には遅いユーザーを救うより高速体験をさらに爆速化すべきという逆説的な教訓と、その背景にある「ひやかし層・きまぐれ層・ロイヤル層」モデルを解説する。
id: miyanaga
...
::: warning 📢 モデルの改訂について
本記事で使用している予測モデルには、**選択バイアスによる歪み**が含まれると判明した。平均LCPが良好でも離脱したユーザーを「冷やかし層」として分析対象から排除した結果、残った「気まぐれ層」において高速体験ができたユーザーのパフォーマンスを過大評価してしまっていた。
...
以下のグラフを見ていただきたい。これは実際の通販サイトにおいてサイトスピードに行動を左右されうるユーザー群の **平均LCPと注文成約率(以下CVR)の関係** を示したものだ。
...
ユーザーが体験するサイトスピード(今回はLCP)について、まず理解いただきたい事実がある。私たちが思う以上にバラバラなのだ。
...
それでも努力する意味はある。早いサイトと遅いサイトには歴然とした差があるからだ。

## サイトスピードに影響を受けないユーザーとは？

ちょっと立ち止まってユーザーの立場で思考実験してみると、実は「**多少のスピードの変化はCVRに影響を及ぼさないユーザー**」もいる。

対象となるのは、サイトスピードが速くても離脱したユーザーと、遅くても注文を成約したユーザーだ。
...
したがって平均LCP 5秒の気の毒なユーザーを3秒に改善するより、平均LCP 1.0秒の十分高速なユーザーを0.9秒に短縮するほうがCVRを一気に押し上げる。
...
先ほどのグラフでは、わずかなLCPの悪化によるCVR急落が顕著に表れている。それは売上専有率におけるスピード格差も示すものだ。
