...
description: AWSなど海外クラウドのデータ送信料金を削減し、デジタル赤字縮小にも貢献するWeb画像軽量化の経済的メリットを解説。月100万PVのサイトで30〜40%のコスト削減効果が見込め、次世代画像フォーマットWebP/AVIFへの移行が「三方よし」の施策になる理由を論じる。
...
通常、ユーザーへ送信するデータの中で割合が最も大きいのは画像である。したがって画像を軽量化すると送信データ量が減り、データ送信料金は安くなる。
...
現在、主流となっている次世代画像フォーマットは、WebP または AVIF である。特に WebP の対応ブラウザシェアは 97%を超えており、たとえば今後 Web 画像を WebP で出力するという判断もありえる。
...
このサービスのメリットは、既存の Web サーバーや Amazon S3 バケットへ変更を一切加えることなく、AWS から送信する画像を WebP へ変換できる点にある。開発作業は不要で、導入したその日から AWS のデータ送信料金を削減できる。
