...
description: JPEG・PNG・GIFの内部フォーマットバリエーション（Exifの有無・圧縮方式・サブサンプリング等）を包括的に網羅したデータセットとPython製生成ツールをOSSとして公開。画像変換ツールの品質評価や予期せぬトラブルの事前検知に活用できる実用的なテストリソースを紹介する。
id: miyanaga
...
画像処理サービスにおいては、実運用時になって思わぬトラブルが報告されるケースもある。
