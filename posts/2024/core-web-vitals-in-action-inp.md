...
description: 'INP（Interaction to Next Paint）はシミュレーションできず改善が難しいCore Web Vitals指標だ。'
...
categories:
  ...
  - technology
  - research
ogp: /ogp/2024/core-web-vitals-in-action-inp.jpg
ads:
  - id: speedismoney
---
...
実例こそ多くないが、長時間を要する処理としては以下のような例があった。
...
イベントハンドラの実行を契機に、DOMの変更に基づくレイアウトの再計算(レンダリング)が行われる。その結果が実際に表示される。
...
ユーザーが遅いINPを経験した後、そのINPの原因要素・操作、時間の要素であるInput delay、Processing time、Presentation delayの値を確認できる。
...
つまりブラウザは一度にひとつのタスクしか実行できない。タスクはひとつひとつが短時間で細切れになっており、それが高速で切り替わる。それで人間にとっては、複数の処理がリアルタムかつ同時並行に感じられる。
...
右肩に赤い三角マークのついたタスクが、50ms以上を要するロングタスクである。
...
サードパーティタグから提供される社外のJavaScriptは、上記のように柔軟な変更はできない。
