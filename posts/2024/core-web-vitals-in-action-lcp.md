...
categories:
  ...
  - technology
  - research
ogp: /ogp/2024/core-web-vitals-in-action-lcp.jpg
ads:
...
Core Web Vitalsには含まれないが、LCPと似た指標としてFCP(First Contentful Paint)がある。文字通り、
...
これは何も特別な話ではなく、ブラウザは本来そのように動作するよう作られている。その足を引っ張ったり、邪魔をする要素があるからFCPが悪化する。
...
ブラウザは内部で、HTMLとCSSを元に画面の表示内容について詳細な設計図を描く(レンダリング)。
...
これらは通常、瞬間的に行われる。とはいえドメインが同一であれば動作はスムーズだ。PageSpeed Insightsの計測でも、FCPで0.5秒ほどの差が確認できる。
