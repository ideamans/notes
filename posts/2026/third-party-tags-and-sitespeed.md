...
IT情報サイトの [ASCII.jp](https://vigilante.sitespeed.info/2026/ascii1.html) でも、全298リソースの約7割（203件）がタグ由来だった。これらを外すと `LCP` は11.6秒から0.4秒へ、`総合スコア` は71から100へと変化した。リソースの7割を占めるタグを外しただけで、スコアが満点に届いた。
...
- サイトスピードが遅いとき、**最初に疑うべきはサードパーティタグ**だ。ページ本体は十分に軽いのに、タグだけで `LCP` が10秒以上延び、Lighthouseの `総合スコア` が40ポイント以上動く例も実在する。
