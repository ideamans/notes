...
CLIを用意するところまではよい。問題はその先で、エージェントはそのCLIの使い方を知らない。
...
リポジトリ内のMarkdownを1か所にまとめ、そこから4つの形式へ派生させる。やっていることはそれだけだ。
...
実例で見た方が早い。CrUXのデータを引く自社CLI `crux-cli` から、この仕組みに関わる部分だけを抜き出す。
...
先頭に鉄則を置くのが型だ。「エージェントが一番間違えること」から書き始める。crux-cli であれば、2つのデータソースを取り違えることと、`0` を「速い」と読んでしまうことがそれにあたる。
...
この形の利点は、実行中のバイナリとバージョンが必ず一致することだ。ドキュメントサイトと違って古くならない。ネットワークも要らない。
...
配布するスキルは、frontmatterと本文からなる1枚のMarkdownである。crux-cli の実物を見てほしい。
...
やりたいのはそこではない。Claudeにスキルを入れ、CLIが無ければ「ついでに入れておいて」と頼めば済む状態にしたかった。そこで `skillcheck` の `RequireInstallSkill` で、配布するスキルには必ず1本含めるようにした。
