...
そのうえで本題はもっと初歩的な話だ。**ファーストビューの最大画像、つまりLCP要素にlazyloadが付いている。** これは高度なチューニングの失敗ではなく、ほとんどの場合は単なるケアレスミスだと思われる。

冷静に考えれば、ページを開いた瞬間から見えている画像に「あとで読む」と指定する理由はない。それでも実例研究では、このミスが繰り返し見つかった。多いのは、かつて全画像へ一律でJavaScript lazyloadをかけた名残が、LCP画像にも残っているパターンだ。標準の `loading="lazy"` をファーストビュー画像にまでうっかり付けている例もある。原因は技術的な難しさではなく見落としだろう。だからこそ数文字の修正でLCPが大きく改善する。
...
- **LCP画像へのlazyloadはたいてい見落としによるものだ。** `loading="lazy"` やJavaScript lazyload（lazysizes等）がファーストビューの最大画像に残っていると、リクエスト開始が大幅に遅れてLCPが悪化する。冷静に見れば気づけるはずの取りこぼしだろう。
